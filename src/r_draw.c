...
fixed_t dc_topstep, dc_iscale, dc_texturemid;
UINT8 *dc_source;
...
void R_InitViewBuffer(INT32 width, INT32 height)
{
	...
	for (i = 0; i < height; i++)
	{
		ylookup[i] = ylookup1[i] = screens[SCREEN_MAIN] + (i+viewwindowy)*vid.width*bytesperpixel;
		ylookup2[i] = screens[SCREEN_MAIN] + (i+(vid.height>>1))*vid.width*bytesperpixel; // for splitscreen
	}
}

/**	\brief	The R_VideoCopy function
...
*/
void R_VideoCopy(size_t ofs, INT32 count)
{
	// LFB copy.
	...
	//  at one point.
	M_Memcpy(screens[SCREEN_MAIN] + ofs, screens[SCREEN_ALTBLIT] + ofs, count);
}
