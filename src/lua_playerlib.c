...
static int player_get(lua_State *L)
{
	...
	player_t *plr = *((player_t **)lua_touserdata(L, 1));// test
	//const char *field = luaL_checkstring(L, 2);
	enum player_e field = Lua_optoption(L, 2, NULL, player_opt, player_fields_ref);
	//timetaken = I_GetTimeNanos() - timetaken;
	//if (gametic % 39 == 0)
	//	CONS_Printf("player_get lookup took %d\n", timetaken);
...
int LUA_PlayerLib(lua_State *L)
{
	...
	player_fields_ref = Lua_createoptiontable(L, player_opt);
