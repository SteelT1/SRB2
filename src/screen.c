...
void SCR_DisplayTicRate(void)
{
	...
	if (fpstime - lastupdatetime > 1000000 * FPSUPDATERATE)
	{
		...
		fpssamples[fpssampleslen-1] = I_GetTimeMicros() - fpstime;
		fpssum += fpssamples[fpssampleslen-1];

		aproxfps = 1000000 / ((double)fpssum / fpssampleslen);

		lastupdatetime = I_GetTimeMicros();
	}

	fpstime = I_GetTimeMicros();
