...
static void R_DrawWallSplats(void)
{
	...
	for (; splat; splat = splat->next)
	{
		...
		for (dc_x = x1; dc_x <= x2; dc_x++, spryscale += rw.scalestep)
		{
			...
			texturecolumn = rw.offset2 - splat->offset - FixedMul(FINETANGENT(angle), rw.distance);
...
static void R_Render2sidedMultiPatchColumn(column_t *column)
{
	...
	if (dc_topstep>>FRACBITS <= mceilingclip[dc_x])
		dc_topstep = mceilingclip[dc_x]<<FRACBITS;
...
void R_RenderMaskedSegRange(drawseg_t *ds, INT32 x1, INT32 x2)
{
	...
	if (curline->polyseg && curline->polyseg->translucency > 0 && cv_translucency.value)
	{
...
static void R_RenderSegLoop (void)
{
	...
	for (; rw.x1 < rw.x2; rw.x1++)
	{
		// mark floor / ceiling areas
		yl = (topfrac+HEIGHTUNIT-1)>>HEIGHTBITS;

		// no space above wall?
		top = ceilingclip[rw.x1]+1;
		...
		if (markceiling)
		{
			...
			if (bottom >= floorclip[rw.x1])
				bottom = floorclip[rw.x1]-1;

			if (top <= bottom)
			{
				ceilingplane->top[rw.x1] = (INT16)top;
				ceilingplane->bottom[rw.x1] = (INT16)bottom;
			}
		}
		...
		if (markfloor)
		{
			...
			{
				floorplane->top[rw.x1] = (INT16)top;
				floorplane->bottom[rw.x1] = (INT16)bottom;
			}
		}

		if (numffloors)
		{
			firstseg->frontscale[rw.x1] = frontscale[rw.x1];
			top = ceilingclip[rw.x1]+1;
			bottom = floorclip[rw.x1]-1;
			...
			for (i = 0; i < numffloors; i++)
			{
				...
				if (curline->polyseg)
				{
					if (ffloor[i].plane->minx > rw.x1)
						ffloor[i].plane->minx = rw.x1;
					else if (ffloor[i].plane->maxx < rw.x1)
						ffloor[i].plane->maxx = rw.x1;
				}
#endif
				...
				if (ffloor[i].height < viewz)
				{
					...
					INT32 bottom_w = ffloor[i].f_clip[rw.x1];
					...
					if (curline->polyseg && ffloor[i].polyobj && ffloor[i].polyobj == curline->polyseg && top_w >= bottom_w)
						ffloor[i].plane->top[rw.x1] = ffloor[i].plane->bottom[rw.x1] = 0xFFFF;
					else
#endif

					if (top_w <= bottom_w)
					{
						ffloor[i].plane->top[rw.x1] = (INT16)top_w;
						ffloor[i].plane->bottom[rw.x1] = (INT16)bottom_w;
					}
				}
				else if (ffloor[i].height > viewz)
				{
					INT32 top_w = ffloor[i].c_clip[rw.x1] + 1;
		...
		angle = (rw.centerangle + xtoviewangle[rw.x1])>>ANGLETOFINESHIFT;
		...
		if (midtexture)
		{
			...
			if (yl <= yh && yh >= 0 && yl < viewheight)
			{
				...
				ceilingclip[rw.x1] = (INT16)viewheight;
				floorclip[rw.x1] = -1;
			}
			else
			{
				...
				if (markceiling)
					ceilingclip[rw.x1] = (yl >= 0) ? ((yl > viewheight) ? (INT16)viewheight : (INT16)((INT16)yl - 1)) : -1;
				if (markfloor)
		...
		else
		{
			...
			if (toptexture)
			{
				...
				if (mid >= floorclip[rw.x1])
					mid = floorclip[rw.x1]-1;
			...
			if (bottomtexture)
			{
				...
				// no space above wall?
				if (mid <= ceilingclip[rw.x1])
					mid = ceilingclip[rw.x1]+1;
				...
				if (mid <= yh) // back floor higher than front floor ?
				{
					...
					else if (mid < viewheight) // safe to draw bottom texture
					{
						...
						colfunc();
						floorclip[rw.x1] = (INT16)mid;
					}
					else  // entirely off bottom of screen
						floorclip[rw.x1] = (INT16)viewheight;
				}
				else
					floorclip[rw.x1] = (yh < viewheight) ? ((yh < -1) ? -1 : (INT16)((INT16)yh + 1)) : (INT16)viewheight;
			}
		...
		if (maskedtexture || numthicksides)
		{
			...
			if (maskedtextureheight != NULL) {
				maskedtextureheight[rw.x1] = (!!(curline->linedef->flags & ML_DONTPEGBOTTOM) ^ !!(curline->linedef->flags & ML_EFFECT3) ?
		...
		{
			ffloor[i].f_clip[rw.x1] = ffloor[i].c_clip[rw.x1] = (INT16)((ffloor[i].b_frac >> HEIGHTBITS) & 0xFFFF);
...
#undef ADJUST
	...
	else
	{
		...
		if (frontsector && backsector && frontsector->tag != backsector->tag && (backsector->ffloors || frontsector->ffloors))
		{
			...
			ds_p->thicksidecol = maskedtexturecol = lastopening - rw.x1;
			lastopening += rw.x2 - rw.x1;
		...
		if (sidedef->midtexture > 0 && sidedef->midtexture < numtextures)
		{
			...
			{
				ds_p->maskedtexturecol = maskedtexturecol = lastopening - rw.x1;
				lastopening += rw.x2 - rw.x1;
			}
			else
	...
	if (segtextured)
	{
		offsetangle = rw.normalangle - rw.angle1;
	...
	topfrac = (centeryfrac>>INVHGTBITS) - FixedMul(worldtop, rw.scale);

	bottomstep = -FixedMul(rw.scalestep, worldbottom);
	bottomfrac = (centeryfrac>>INVHGTBITS) - FixedMul(worldbottom, rw.scale);
	...
	if (frontsector->numlights)
	{
		...
		for (i = p = 0; i < dc_numlights; i++)
		{
...
#ifdef ESLOPE
			rlight->height = (centeryfrac>>INVHGTBITS) - FixedMul(leftheight, rw.scale);
			rlight->heightstep = (centeryfrac>>INVHGTBITS) - FixedMul(rightheight, ds_p->scale2);
			rlight->heightstep = (rlight->heightstep-rlight->height) / range;
#else
			rlight->height = (centeryfrac>>INVHGTBITS) - FixedMul((light->height - viewz) >> INVHGTBITS, rw.scale);
			rlight->heightstep = -FixedMul(rw.scalestep, (light->height - viewz) >> INVHGTBITS);
#endif
			rlight->flags = light->flags;
			...
			if (light->caster && light->caster->flags & FF_CUTSOLIDS)
			{
				...
				rlight->botheight = (centeryfrac>>INVHGTBITS) - FixedMul(leftheight, rw.scale);
				rlight->botheightstep = (centeryfrac>>INVHGTBITS) - FixedMul(rightheight, ds_p->scale2);
				rlight->botheightstep = (rlight->botheightstep-rlight->botheight)/ range;

#else
				rlight->botheight = (centeryfrac >> INVHGTBITS) - FixedMul((*light->caster->bottomheight - viewz) >> INVHGTBITS, rw.scale);
				rlight->botheightstep = -FixedMul(rw.scalestep, (*light->caster->bottomheight - viewz) >> INVHGTBITS);
#endif
			}
	...
	if (numffloors)
	{
		for (i = 0; i < numffloors; i++)
		{
			...
			ffloor[i].f_frac = (centeryfrac>>INVHGTBITS) - FixedMul(ffloor[i].f_pos, rw.scale);
	...
	if (backsector)
	{
		...
		if (toptexture)
		{
			pixhigh = (centeryfrac>>INVHGTBITS) - FixedMul(worldhigh, rw.scale);
		...
		if (bottomtexture)
		{
			pixlow = (centeryfrac>>INVHGTBITS) - FixedMul (worldlow, rw.scale);
			...
			if (backsector->ffloors)
			{
				for (rover = backsector->ffloors; rover && i < MAXFFLOORS; rover = rover->next)
				{
					     ...
					     (viewz > planevistest && (rover->flags & FF_BOTHPLANES))))
					{
						...
						ffloor[i].b_frac = (centeryfrac >> INVHGTBITS) - FixedMul(ffloor[i].b_pos, rw.scale);
	...
	if (numffloors)
	{
		if (!firstseg)
		{
			...
			for (i = 0; i < numffloors; i++)
				ds_p->ffloorplanes[i] = ffloor[i].plane = R_CheckPlane(ffloor[i].plane, rw.x1, rw.x2 - 1);
		...
		{
			for (i = 0; i < numffloors; i++)
				R_ExpandPlane(ffloor[i].plane, rw.x1, rw.x2 - 1);
		}
	}
	...
	{
		M_Memcpy(lastopening, ceilingclip+start, 2*(rw.x2 - start));
		ds_p->sprtopclip = lastopening - start;
		lastopening += rw.x2 - start;
	}

	if (((ds_p->silhouette & SIL_BOTTOM) || maskedtexture) && !ds_p->sprbottomclip)
	{
		M_Memcpy(lastopening, floorclip + start, 2*(rw.x2-start));
